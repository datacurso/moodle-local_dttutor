...
$string['content_chars_per_activity'] = 'Символов материала на занятие';
$string['content_chars_per_activity_desc'] = 'Сколько от одного занятия передаётся вместе с вопросом. Более длинный текст обрезается, и репетитор получает предупреждение, что читает выдержку.';
$string['content_chars_total'] = 'Символов материала на вопрос';
$string['content_chars_total_desc'] = 'Сколько материала передаётся с каждым вопросом суммарно по всем занятиям. Это ограничивает стоимость каждого ответа в кредитах.';
$string['course_identity'] = 'Облик репетитора в этом курсе';
$string['course_identity_help'] = 'Оставьте поле пустым, чтобы использовать настройку сайта. Институциональные указания о тоне и ограничениях задаются для всего сайта и здесь не меняются.';
$string['course_tutorname'] = 'Имя репетитора в этом курсе';
$string['course_tutorname_help'] = 'Как зовут репетитора в заголовке чата этого курса. Пусто — значит имя, заданное для сайта. Подстановки сайта работают и здесь.';
$string['course_welcomemessage'] = 'Приветствие в этом курсе';
$string['course_welcomemessage_help'] = 'Первое сообщение чата в этом курсе. Пусто — значит сообщение, заданное для сайта. Подстановки сайта работают и здесь.';
$string['credits_left'] = 'Остаток кредитов по лицензии';
$string['credits_unknown'] = 'Сейчас не удалось получить остаток кредитов.';
...
$string['enabled_by_default'] = 'Включать репетитора в новых курсах';
$string['enabled_by_default_desc'] = 'Новые курсы начинаются с включённым репетитором, поэтому развёртывание не требует обхода каждого курса. Преподаватель может отключить его в своём курсе.';
...
$string['error_provider_disabled'] = 'ИИ-репетитор недоступен, так как поставщик ИИ отключён на этом сайте. Обратитесь к администратору.';
$string['error_quiz_in_progress'] = 'ИИ-репетитор недоступен, пока открыта попытка теста. Отправьте её или дождитесь окончания времени — и репетитор вернётся.';
$string['error_ratelimit_exceeded'] = 'Превышен допустимый лимит использования. Пожалуйста, попробуйте снова {$a}.';
$string['error_tutor_disabled_site'] = 'ИИ-репетитор отключён на этом сайте. Обратитесь к администратору сайта, если он вам нужен.';
...
$string['event_service_failed'] = 'Сбой сервиса ИИ';
$string['event_tutor_used'] = 'ИИ-репетитор использован';
$string['include_content'] = 'Отправлять материалы курса ИИ-репетитору';
$string['include_content_desc'] = 'По умолчанию выключено. Во включённом состоянии репетитор получает также текст, написанный преподавателями: описание каждого занятия, содержимое страницы, видимые главы книги, инструкции задания и адрес URL. Он никогда не получает то, что пишут обучающиеся: сообщения форума, статьи глоссария, записи базы данных, страницы вики или ответы на задания, а также вопросы теста и страницы лекции. Передаются только материалы занятий, которые пользователь уже может открыть. Отправка материалов курса во внешний сервис — решение ответственного за защиту данных.';
...
$string['max_activities'] = 'Максимум занятий, отправляемых репетитору';
$string['max_activities_desc'] = 'Сколько занятий курса передаётся сервису ИИ с каждым вопросом. В большом курсе отправка всех занятий удорожает каждый ответ и быстрее расходует кредиты. О невключённых занятиях репетитор получает уведомление, чтобы он мог сказать об этом, а не отрицать их существование.';
$string['new_conversation'] = 'Новый диалог';
...
$string['position_overlap_warning'] = 'Эта позиция совпадает с местом, где Moodle размещает свои плавающие элементы, например сообщения. Кнопка может перекрыть их или оказаться под ними.';
...
$string['recent_failures'] = 'Сбои сервиса ИИ';
$string['recent_failures_desc'] = 'За последние сутки репетитор зафиксировал сбоев сервиса ИИ: {$a}. Найдите событие «Сбой сервиса ИИ» в журналах сайта.';
...
$string['retention_days'] = 'Дней хранения диалогов';
$string['retention_days_desc'] = 'Более старые диалоги удаляются в Moodle и в сервисе ИИ. Ноль сохраняет их до удаления пользователя, удаления курса или запроса на удаление данных.';
$string['selected'] = 'выбрано';
$string['sendmessage'] = 'Отправить сообщение';
$string['service_available'] = 'Сервис ИИ доступен.';
$string['service_status'] = 'Состояние сервиса';
$string['service_unavailable'] = 'Сервис ИИ недоступен: поставщик ИИ отключён на этом сайте.';
$string['student'] = 'Студент';
$string['task_purge_old_conversations'] = 'Удалять диалоги по истечении срока хранения';
...
$string['usage_in_course'] = 'Задано вопросов в этом курсе за последние {$a->days} дн.: {$a->questions}.';
$string['usage_unknown'] = 'Использование репетитора подсчитать нельзя: сайт не ведёт доступный журнал.';
